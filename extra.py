...
def home(): 
    ...
    national_last7days_cases, national_current_hospital_cases, national_total_deaths = process_covid_csv_data(
        parse_csv_data(r"<INSERT PATH TO covid_API_data.csv HERE>"))
    update_name = request.args.get("two")
    news_info = get_news()
    request_to_delete_news = request.args.get("notif")
    if request_to_delete_news != None:
        logging.info("A request to delete news has been placed")
        delete_title = request_to_delete_news.split("+")
        delete_title[0].replace("['", "")
        delete_title[0].replace("']", "")
        unwanted_news.append(delete_title[0])
    elif request_to_delete_news == "":
        if len(unwanted_news) != 0:
            previously_deleted.append(unwanted_news[-1])
    seen = set()
    news_articles = [] 
    for d in news_info:
        t = tuple(d.items())
    ...
    if request_to_delete_news != None:
        for i in range(len(delete_title)):
            for i in news_articles:
                for value in i.values():
                    if value == unwanted_news[-1]:
                       index = news_articles.index({'title':  unwanted_news[-1]})
                       news_articles.pop(index)

    elif request_to_delete_news == None:
        if len(previously_deleted) != 0:
            for i in range(len(delete_title)):
                for i in news_articles:
                    for value in i.values():
                        index = news_articles.index({'title':  previously_deleted[-1]})
                        news_articles.pop(index)                        
    if update_name:
